...
def index():
    if request.method=="POST":
        ...
        s2="The score of credit card upgrade base on logistic regression is " +str(pred2)
        
    
        model3=joblib.load("CCU_NN")
        pred3=model3.predict([[purchases, suppcard]])
        s3="The score of credit card upgrade base on neural network model is " +str(pred3)
        
        model4=joblib.load("CCU_RF")
        pred4=model4.predict([[purchases, suppcard]])
        s4="The score of credit card upgrade base on random forest model is " +str(pred4) 
        
        
        model5=joblib.load("CCU_GB")
        pred5=model5.predict([[purchases, suppcard]])
        s5="The score of credit card upgrade base on gradient boosting model is " +str(pred5) 
